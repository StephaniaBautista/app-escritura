...
describe('PostItWall', () => {
  ...
  it('carga notas del documento y del proyecto al montar', async () => {
    render(<PostItWall documentId="doc-1" rootDocumentId="doc-1" projectId="proj-1" />)

    await waitFor(() => {
      expect(baseStore.loadNotes).toHaveBeenCalledWith('doc-1', 'doc-1')
  ...
  it('filtra solo notas del documento', () => {
    useDocumentStoreMock.mockReturnValue({
      ...baseStore,
      notes: [makeNote('a', { documentId: 'doc-1' })],
      projectNotes: [makeNote('b')],
    })
  ...
  it('crea nota de documento desde el menú +', async () => {
    render(<PostItWall documentId="doc-1" rootDocumentId="doc-1" projectId="proj-1" />)

    fireEvent.click(screen.getByRole('button', { name: 'postit.newNote' }))
    fireEvent.click(screen.getByRole('button', { name: 'postit.scopeDocument' }))
    const input = screen.getByPlaceholderText('postit.notePlaceholder')
    fireEvent.change(input, { target: { value: 'Regla del documento' } })
    fireEvent.keyDown(input, { key: 'Enter' })

    await waitFor(() => {
      expect(baseStore.createNote).toHaveBeenCalledWith('doc-1', { title: 'Regla del documento' })
    })
  })
